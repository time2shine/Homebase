...
Most data stays on your device. Some features make network requests to fetch
content (news feeds, weather, and wallpaper media).

## Data the extension stores locally
Homebase stores the following locally in your browser (via browser storage and/or localStorage):
...
- History (to show recent items and improve suggestions, if enabled)
- Tabs (to open links in new tabs / manage opening behavior)
- Firefox Containers (to open items in a specific container, if enabled)

Homebase does not transmit your bookmarks or history contents to a server.

## Clipboard access
Homebase reads clipboard text only when you explicitly trigger a paste action
...
such as Al Jazeera, BBC RSS, Feedburner feeds, ESPN, and Cricinfo feeds.
